...
function reconnect()
    ...
    data, err = socket.connect(host, port, nil, nil, "AF_INET6")
...
control, err = socket.connect(host, port, nil, nil, "AF_INET6")
if err then fail(err)
else pass("connected!") end
...
function empty_connect()
    ...
    data, err = socket.connect("", port, nil, nil, "AF_INET6")
    if not data then 
        pass("ok")
        data = socket.connect(host, port, nil, nil, "AF_INET6")
    else 
...
print(r, s, e)
    ...
    for i = 1, socket._SETSIZE+1 do
        toomany[#toomany+1] = socket.udp("AF_INET6")
    end
...
function accept_timeout()
    ...
    local s, e = socket.bind("*", 0, 0, "AF_INET6")
    assert(s, e)
    local t = socket.gettime()
    s:settimeout(1)
    local c, e = s:accept()
...
function connect_timeout()
    ...
    local t = socket.gettime()
    local c, e = socket.tcp("AF_INET6")
    assert(c, e)
    c:settimeout(0.1)
    local t = socket.gettime()
    local r, e = c:connect("10.0.0.1", 81, "AF_INET6")
print(r, e)
...
function accept_errors()
    ...
    local d, e = socket.bind("*", 0, nil, "AF_INET6")
    assert(d, e);
    local c, e = socket.tcp("AF_INET6");
    assert(c, e);
    d:setfd(c:getfd())
...
function connect_errors()
    ...
    local c, e = socket.connect("localhost", 1, nil, nil, "AF_INET6");
    assert(not c and e)
    print("ok: ", e)
    io.stderr:write("host not found: ")
    local c, e = socket.connect("host.is.invalid", 1, nil, nil, "AF_INET6");
...
function rebind_test()
    local c = socket.bind("localhost", 0, nil, "AF_INET6")
    local i, p = c:getsockname()
    local s, e = socket.tcp("AF_INET6")
    assert(s, e)
...
test_methods(socket.tcp("AF_INET6"), {
    "accept",
    "bind",
    "close",
    "connect",
...
test_methods(socket.udp("AF_INET6"), {
    "close", 
    "getpeername",
    "dirty",
    "getfd",
